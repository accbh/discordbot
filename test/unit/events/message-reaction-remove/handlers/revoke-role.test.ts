...
describe('RevokeRoleHandler', () => {
    ...
    beforeEach(() => {
        ...
        handler = new RevokeRoleHandler(roleName, messageId, emojiName, logger);
    });
    ...
    describe('handle', () => {
        ...
        let extractMessagePropsStub: sinon.SinonStub;
        ...
        beforeEach(() => {
            ...
            extractMessagePropsStub = sandbox.stub(handler, 'extractMessageProps')
                .resolves({ member, role });
        });

        it('should reject with any error thrown by handler.extractMessageProps', () => {
            const error = new Error('Some fake error');
            extractMessagePropsStub.throws(error);

            return handler.handle(messageReaction, user)
                .then(() => {
                    throw new Error('Expected an error to be thrown but got success');
                }, err => {
                    extractMessagePropsStub.should.have.been.calledOnce;
                    extractMessagePropsStub.should.have.been.calledWithExactly(messageReaction.message, user, roleName);
